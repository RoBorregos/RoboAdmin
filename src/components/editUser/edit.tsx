...
interface EditProps {
  ...
  image?: string;
  add?: boolean;
  onBeforeAdd?: (newId: number) => Promise<boolean>;
}
...
const Edit: React.FC<EditProps> = ({ data, add, onBeforeAdd }) => {
  ...
  const handleEdit = async (values: FormValues) => {
    if (values) {
      if (add) {
        ...
        const res = await onBeforeAdd?.(newId);
  ...
  return (
    <>
      <div className="h-full w-full overflow-y-auto p-6 pb-16">
        ...
        <Formik
          ...
          onSubmit={async (values, actions) => {
            await handleEdit(values as FormValues);
